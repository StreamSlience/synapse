...
Synapse ships a **vendored Node runtime** alongside the app. Because Node 22.5+
...
synapse-<target>/
  ...
  bin/
    synapse | synapse.cmd   # launcher → runs the bundled Node with the app
```
...
1. **`curl | sh`** ([`install.sh`](install.sh)) — no Node required; ideal for a
   ...
   Releases, symlinks `synapse` onto PATH. Re-run to upgrade; `--uninstall` to
   remove.
2. **npm** ([`scripts/npm-shim.js`](scripts/npm-shim.js)) — preserves
   `npm i -g @colbymchenry/synapse`. The main package is a tiny shim; the
   bundles ship as per-platform `optionalDependencies`
   (`@colbymchenry/synapse-<target>` with `os`/`cpu`), so npm installs only the
...
- Retire the now-vestigial Node-version gate in `src/bin/synapse.ts` — the
