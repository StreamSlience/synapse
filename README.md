...
Already installed? Run `synapse upgrade` to update in place.

Follow [@getsynapse](https://x.com/getsynapse) on X for updates.

### Supercharge Claude Code, Cursor, Codex, OpenCode, Hermes Agent, Gemini, Antigravity, and Kiro with Semantic Code Intelligence

**~16% cheaper · ~58% fewer tool calls · 100% local**

### [Documentation & Website →](https://colbymchenry.github.io/synapse/)
...
**The Synapse platform is coming** — for every PR, know exactly what to test, what could break, which flows are affected, and whether business logic is compromised.

<a href="https://getsynapse.com"><img alt="Join the waitlist for early beta access" src="https://raw.githubusercontent.com/colbymchenry/synapse/main/assets/waitlist.svg?v=2" height="52"></a>

<sub>Get <b>early beta access</b> to the hosted product · <a href="https://getsynapse.com">getsynapse.com</a></sub>
...
<sub>Synapse bundles its own runtime — nothing to compile, no native build, works the same everywhere. The installer puts `synapse` on your PATH but **doesn't change your current shell** — open a new terminal before the next step so the command resolves.</sub>

<sub>**Upgrade any time** with `synapse upgrade` — it detects how you installed (bundle, npm, or npx) and updates in place. Add `--check` to see if an update is available, or `synapse upgrade <version>` to pin one.</sub>

</details>

### 2. Wire up your agent(s)

In a **new terminal**, run the installer to connect Synapse to the agents you use:

```bash
synapse install
```

<sub>Detects and auto-configures Claude Code, Cursor, Codex CLI, opencode, Hermes Agent, Gemini CLI, Antigravity IDE, and Kiro — wiring the Synapse MCP server into each. **This is the step that connects Synapse to your agent;** installing the CLI in step 1 does not do it on its own. (Shortcut: `npx @colbymchenry/synapse` downloads and runs this in one go.)</sub>
...
<sub>`synapse init` creates the local `.synapse/` directory and builds the full graph in the same step — one command, done.</sub>
...
Auto-sync is enabled by default. Synapse watches the project and updates the graph on every file change — while your agent edits code, or you add, modify, or delete files. **The index is never stale, and there is nothing to re-run.**
...
<sub>Reverses the installer — strips Synapse's MCP server config, instructions, and permissions from each configured agent. Your project indexes (`.synapse/`) are left untouched; remove those per-project with `synapse uninit`. Use `--target` to remove from specific agents, or `--yes` to run non-interactively.</sub>
...
**Synapse gives those agents a pre-indexed knowledge graph** — symbol relationships, call graphs, and code structure. Agents query the graph instantly instead of scanning files.

### Benchmark Results

Tested across **7 real-world open-source codebases** spanning 7 languages, comparing an agent (Claude Code, headless) answering one architecture question **with** and **without** Synapse. Each cell is the savings at the **median of 4 runs per arm**. _Re-validated on Opus 4.8 (2026-06-02), on the current build (`synapse_explore` as the primary tool)._
...
Synapse cuts **tokens, tool calls, and wall-clock time on every repo** — across small, medium, and large codebases — and answers them with **near-zero file reads**, while the no-Synapse agent spends its budget on grep/find/Read discovery. `synapse_explore` shows the answer in full — the mechanism plus the exact methods you asked about, even when they're buried in a multi-thousand-line file — while collapsing redundant interchangeable implementations to signatures, so the response is sized to the *answer* rather than the file count. **Cost stays flat-to-cheaper everywhere** — largest on the small repos (Alamofire, OkHttp), roughly break-even on the most response-heavy ones (Excalidraw, Tokio), where Synapse trades the no-Synapse agent's many small grep/read round-trips for a few large, cache-heavy tool responses.
...
**Methodology.** Each arm is `claude -p` (Claude Opus 4.8) run headlessly against the repo with `--strict-mcp-config`: **WITH** = Synapse's MCP server enabled, **WITHOUT** = an empty MCP config. Built-in Read/Grep/Bash stay available to both. Same question per repo, **4 runs per arm, median reported**. Cost = the run's `total_cost_usd`; Tokens = total tokens processed (input incl. cached + output); Time = wall-clock; Tool calls = every tool invocation, including those inside any sub-agents the model spawns. Repos cloned at `--depth 1` and indexed by the same Synapse build that served them. Re-validated 2026-06-02 on the current build. These numbers are lower than the prior Opus 4.7 validation — not a Synapse regression but a stronger native baseline: Opus 4.8 greps/reads efficiently on the main thread instead of fanning out into large Explore-subagent sweeps, so the no-Synapse arm is leaner than it used to be. Per-repo numbers move run-to-run with how hard the without-arm thrashes (the median-of-4 smooths it, but tails remain — e.g. Django's without-arm hit $2.71/14m one batch).
...
**Why Synapse wins:** with the index available, the agent answers directly — usually one `synapse_explore` returns the relevant source — and stops, usually with zero file reads. Without it, the agent spends most of its budget on discovery (find/ls/grep) before reading the right code. Synapse only helps when queried *directly*, so its instructions steer agents to answer directly rather than delegate exploration to file-reading sub-agents — otherwise a sub-agent reads files regardless and Synapse becomes overhead.
...
<details>
<summary><strong>How auto-syncing works — and why you don't need to run <code>synapse sync</code> manually</strong></summary>

When your agent (Claude Code, Cursor, Codex, opencode) launches `synapse serve --mcp`, three layers keep the index in step with your code — and make sure the agent never gets a silent wrong answer in the brief window between an edit and the next sync:

1. **File watcher with debounced auto-sync.** A native FSEvents / inotify / ReadDirectoryChangesW watcher captures every source-file create / modify / delete and triggers a re-index after a debounce window (default `2000ms`, tunable via `SYNAPSE_WATCH_DEBOUNCE_MS`, clamped to `[100ms, 60s]`). Bursts of edits collapse into a single sync.
...
**Verify any time** with `synapse_status` (via MCP) or `synapse status` (CLI). If anything is pending, you'll see a `### Pending sync:` section naming the files and their edit age.

The handful of cases where manual `synapse sync` makes sense: the watcher is disabled (sandboxed environments, or `SYNAPSE_NO_DAEMON=1`), or you're scripting against the index outside an agent session and want a pre-flight sync at the start of your script.

→ Full deep-dive in [Guides → Indexing a Project](https://colbymchenry.github.io/synapse/guides/indexing/#stay-fresh-automatically).
...
Synapse detects web-framework routing files and emits `route` nodes linked by `references` edges to their handler classes or functions. Querying callers of a view/controller now surfaces the URL pattern that binds it.
...
- Write each chosen agent's MCP server config, plus a small marker-fenced Synapse section in the agent's instructions file (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`) — that's how subagents and non-MCP agents learn the `synapse explore` / `synapse node` commands, since the MCP server's own guidance only reaches the main agent. Removed cleanly by `synapse uninstall`.
...
Builds the per-project knowledge graph index, which then auto-syncs on every file change. A single global `synapse install` works in every project you open — no need to re-run the installer per project.
...
Synapse's MCP server delivers its usage guidance to your agent **automatically**, in the MCP `initialize` response. In short, it tells the agent to:

- **Answer structural questions directly with Synapse** — it *is* the pre-built index, so a grep/read loop just repeats work it already did. Treat the returned source as already read.
- **Pick the tool by intent:** `synapse_explore` for almost anything — "how does X work", a flow/"how does X reach Y", or surveying an area (one call returns the relevant symbols' source grouped by file); `synapse_search` to just locate a symbol; `synapse_callers` for every call site (including callback registrations); `synapse_node` for one symbol's full source + callers, or to read a file like the Read tool.
- **Trust the results — don't re-verify with grep**, and check the staleness banner after edits.
- In a workspace with no index, Synapse announces itself inactive and serves no tools — indexing stays your decision.

The exact text is `src/mcp/server-instructions.ts` — the single source of truth for the main agent. Because subagents and non-MCP harnesses never see the MCP guidance, the installer also writes a four-line marker-fenced section into the agent's instructions file pointing at the `synapse explore` / `synapse node` CLI equivalents.
...
│       calls Synapse tools directly — no Explore sub-agent       │
│                                 │                                 │
└─────────────────────────────────┬─────────────────────────────────┘
                                  │
                                  ▼
...
2. **Storage** — Everything goes into a local SQLite database (`.synapse/synapse.db`) with FTS5 full-text search.
...
synapse unlock [path]           # Remove a stale lock file that's blocking indexing
synapse query <search>          # Search symbols (--kind, --limit, --json)
synapse explore <query>         # Relevant symbols' source + call paths in one shot (same output as the synapse_explore MCP tool)
synapse node <symbol|file>      # One symbol's source + callers, or read a file with line numbers (same output as synapse_node)
...
synapse affected [files...]     # Find test files affected by changes (see below)
synapse daemon                  # Manage background daemons — pick one to stop (alias: daemons)
synapse telemetry [on|off]      # Show or change anonymous usage telemetry
synapse upgrade [version]       # Update to the latest release (--check, --force)
synapse version                 # Print the installed version (also -v, --version)
synapse help [command]          # Show help, optionally for one command
```
...
When running as an MCP server, Synapse exposes a focused set of four tools — measured agent behavior showed a leaner list steers agents to the right tool and saves context every session:

| Tool | Purpose |
|------|---------|
| `synapse_explore` | **Primary.** Answer almost any question in one call — "how does X work", a flow ("how does X reach Y"), or surveying an area — returning the relevant symbols' verbatim source grouped by file, plus a relationship map and blast radius. Surfaces dynamic-dispatch hops (callbacks, React re-render, interface→impl) grep can't follow. |
| `synapse_node` | One symbol's full source + caller/callee trail (every overload for an ambiguous name) — or pass a file path to **read a whole file like the Read tool** (same line-numbered output, `offset`/`limit`), with its dependents attached. |
| `synapse_search` | Find symbols by name across the codebase |
| `synapse_callers` | Every call site of a function — including where it's registered as a callback — with one section per definition when several share a name |

Four more tools (`synapse_callees`, `synapse_impact`, `synapse_files`, `synapse_status`) stay fully functional but unlisted by default — measured across eval runs, agents never or rarely picked them, and their information already arrives inline on the four above (explore's blast-radius section, node's dependents note, a symbol's body as its callee list). Re-enable any of them with the `SYNAPSE_MCP_TOOLS` environment variable (e.g. `SYNAPSE_MCP_TOOLS=explore,node,search,callers,impact`), or use their CLI equivalents (`synapse callees` / `impact` / `files` / `status`).

In a workspace with no `.synapse/` index, the server announces itself inactive and lists **no** tools — agents work normally with their built-in tools, and indexing stays your decision.

---

## Library Usage

Synapse can be embedded directly. The npm package re-exports its programmatic
API, so both `import` and `require` resolve the `Synapse` class in your own
...
// Or: const cg = await Synapse.open('/path/to/project');
...
- Install from npm (`npm i @colbymchenry/synapse`) so the matching
...
There isn't any — Synapse is zero-config, with **no config file** to write or
...
Synapse collects **anonymous usage statistics** — which tools and commands get
...
```bash
synapse telemetry off    # or: SYNAPSE_TELEMETRY=0, or DO_NOT_TRACK=1
```
...
**"Synapse not initialized"** — Run `synapse init` in your project directory first.

**Indexing is slow** — Check that `node_modules` and other large directories are excluded. Use `--quiet` to reduce output overhead.

**MCP hits `database is locked`** — current builds shouldn't: Synapse bundles its own Node runtime and uses Node's built-in `node:sqlite` in WAL mode, where concurrent reads never block on a writer. If you still see it:
...
**MCP server not connecting** — Your agent starts the server itself, so you don't launch it by hand. Make sure the project is initialized and indexed (`synapse status`) and that the path in your MCP config is correct. If it still won't connect, re-run `synapse install` to rewrite the config.

**Missing symbols** — The MCP server auto-syncs on save (wait a couple seconds). Run `synapse sync` manually if needed. Check that the file's language is supported and isn't inside a `.gitignore`d or default-excluded directory (e.g. `node_modules`, `dist`).

**Sharing one checkout between Windows and WSL** — Don't point both at the same `.synapse/`: the background-server lock and the SQLite index are tied to the OS that wrote them, and SQLite locking across the WSL2/Windows filesystem boundary is unreliable. Give each side its own index in the same tree by setting `SYNAPSE_DIR` to a distinct name on one of them — e.g. `SYNAPSE_DIR=.synapse-win` on Windows, leaving WSL on the default `.synapse`. Synapse skips any sibling `.synapse-*` directory when indexing and watching, so the two never trip over each other.
...
[Report Bug](https://github.com/colbymchenry/synapse/issues) · [Request Feature](https://github.com/colbymchenry/synapse/issues)
