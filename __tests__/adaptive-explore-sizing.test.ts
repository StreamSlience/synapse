/**
 * Regression test for adaptive `synapse_explore` sizing — sibling
 ...
 * >= MIN_SIBLINGS (3) implementers — `synapse_explore` renders it as a
 ...
 * pipeline steps stay full). Default ON; SYNAPSE_ADAPTIVE_EXPLORE=0 disables.
 *
...
describe('adaptive synapse_explore sizing — sibling skeletonization', () => {
  let testDir: string;
  let cg: Synapse;
  ...
  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-adaptive-explore-'));
...
`
  ...
  beforeEach(() => {
    ...
    delete process.env.SYNAPSE_ADAPTIVE_EXPLORE;
  });
  ...
  it('skeletonizes off-spine polymorphic siblings (bodies elided, signatures kept)', async () => {
    const result = await handler.execute('synapse_explore', { query: QUERY, maxFiles: 12 });
  ...
  it('SYNAPSE_ADAPTIVE_EXPLORE=0 disables skeletonization (siblings render full)', async () => {
    process.env.SYNAPSE_ADAPTIVE_EXPLORE = '0';
    try {
      const result = await handler.execute('synapse_explore', { query: QUERY, maxFiles: 12 });
      const text = result.content?.[0]?.text ?? '';
  ...
  it('spares an off-spine sibling when the agent NAMED a callable in it (RealCall fix)', async () => {
    const result = await handler.execute('synapse_explore', { query: SPARE_QUERY, maxFiles: 15 });
    const text = result.content?.[0]?.text ?? '';
    expect(text).toContain('## Flow (call path among the symbols you queried)');
  ...
  it('naming a SHARED/polymorphic method does not spare the siblings (uniqueness-aware)', async () => {
    ...
    const result = await handler.execute('synapse_explore', { query: `${QUERY} intercept`, maxFiles: 12 });
    const text = result.content?.[0]?.text ?? '';
