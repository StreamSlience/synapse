/**
 * `synapse affected` input-path normalization (#825).
 *
...
describe('synapse affected — input path normalization (#825)', () => {
  let tempDir: string;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-affected-paths-'));
