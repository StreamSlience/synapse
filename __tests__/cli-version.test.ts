/**
 * Tests for the `synapse version` affordances.
 *
...
describe('synapse version affordances', () => {
  for (const spelling of ['version', '-v', '-version', '--version', '-V']) {
    it(`\`synapse ${spelling}\` prints exactly the package version`, () => {
  ...
  it('`synapse help` prints usage and the command list', () => {
    const out = run(['help']);
    expect(out).toContain('Usage: synapse');
  ...
  it('a trailing `-v` is still the subcommand\'s --verbose, not the version intercept', () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-version-test-'));
    let combined = '';
    try {
