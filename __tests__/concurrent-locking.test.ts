...
describe('issue #238 — connection PRAGMAs (#1)', () => {
  ...
  beforeAll(() => {
    ...
    conn = DatabaseConnection.initialize(path.join(dir, 'synapse.db'));
  });
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  it('getSynapse(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = vi.spyOn(Synapse, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = (handler as any).getSynapse(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = (handler as any).getSynapse(path.join(root, 'does', 'not', 'exist'));
  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('synapse_search', { query: 'helper' }),
        handler.execute('synapse_search', { query: 'helper', projectPath: root }),
        handler.execute('synapse_callers', { symbol: 'helper', projectPath: root }),
        handler.execute('synapse_callees', { symbol: 'main' }),
        handler.execute('synapse_files', { projectPath: root }),
        handler.execute('synapse_status', { projectPath: root }),
      ];
