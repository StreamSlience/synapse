/**
 * #383 — Synapse indexes config KEYS but must never surface config VALUES.
 *
 ...
 * passwords, API keys, JDBC URLs with embedded creds). Synapse must surface
 * the KEY and never the value — not in node metadata (docstring/signature),
 * not via `synapse_explore`'s verbatim source dump, and not via
 * `synapse_node` `includeCode`. An agent that genuinely needs a value can
 * read the file itself (a deliberate pull); Synapse must never volunteer it.
 */
...
describe('config secret redaction (#383)', () => {
  ...
  it('synapse_explore surfaces the config key but NEVER the secret value', async () => {
    const res = await handler.execute('synapse_explore', {
  ...
  it('synapse_node includeCode returns the key, not the secret value', async () => {
    const res = await handler.execute('synapse_node', {
