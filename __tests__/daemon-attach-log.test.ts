 ...
 * It's now gated behind SYNAPSE_MCP_LOG_ATTACH=1 — silent by default, opt-in
 * for debugging. Approach from #640 by @mturac.
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { logAttachedDaemon } from '../src/mcp/proxy';

const hello = { pid: 4242, synapse: '9.9.9' } as any;
...
describe('daemon attach log gating (#618)', () => {
  ...
  afterEach(() => {
    spy.mockRestore();
    delete process.env.SYNAPSE_MCP_LOG_ATTACH;
  });
  ...
  it('logs the attach line only when SYNAPSE_MCP_LOG_ATTACH=1 (opt-in debug)', () => {
    process.env.SYNAPSE_MCP_LOG_ATTACH = '1';
    logAttachedDaemon('/tmp/cg.sock', hello);
