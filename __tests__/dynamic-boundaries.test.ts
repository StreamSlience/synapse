 ...
 *
 * When the flow an agent asked synapse_explore about does NOT fully connect,
...
// ---------------------------------------------------------------------------
// Integration: synapse_explore output
// ---------------------------------------------------------------------------

describe('synapse_explore — dynamic boundaries', () => {
  let testDir: string;
  let cg: Synapse;
  let handler: ToolHandler;

  const setup = async (files: Record<string, string>, include: string[]) => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-boundary-'));
    const src = path.join(testDir, 'src');
    fs.mkdirSync(src, { recursive: true });
    for (const [name, content] of Object.entries(files)) {
      fs.writeFileSync(path.join(src, name), content);
    }
    cg = Synapse.initSync(testDir, { config: { include, exclude: [] } });
    await cg.indexAll();
  ...
  it('announces the boundary site and shortlists the keyed candidate', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'routeSave onSave' });
  ...
  it('announces a runtime-keyed boundary with no candidate list', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'route onSave' });
    const text = res.content[0].text as string;
  ...
  it('surfaces the boundary even when the other symbol is not in the graph', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'route processPayment' });
    const text = res.content[0].text as string;
    expect(text).toContain('## Dynamic boundaries');
  });
  ...
  it('renders a direct synthesized emit→handler hop as a dynamic-dispatch link (#687 criterion 1)', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'completeCheckout settleInvoice' });
    const text = res.content[0].text as string;
  ...
  it('never adds the section to a fully connected flow', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'stepOne stepThree' });
    const text = res.content[0].text as string;
  ...
  it('python getattr dispatch surfaces with a prefix-key candidate', async () => {
    ...
    const res = await handler.execute('synapse_explore', { query: 'process handle_save' });
    const text = res.content[0].text as string;
