...
if (!fs.existsSync(path.join(resolvedPath, '.synapse', 'synapse.db'))) {
  console.error(`No .synapse/synapse.db found at ${resolvedPath}`);
  process.exit(1);
}

let synapseSha = 'unknown';
try {
  synapseSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nSynapse Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${synapseSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = Synapse.openSync(resolvedPath);
