/**
 * synapse_explore blast-radius section.
 *
...
describe('synapse_explore — blast radius', () => {
  let testDir: string;
  let cg: Synapse;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-blast-'));
  ...
  it('lists dependents (locations only) and covering tests for an entry symbol', async () => {
    const res = await handler.execute('synapse_explore', { query: 'target' });
  ...
  it('omits symbols that have no dependents from the blast radius', async () => {
    const res = await handler.execute('synapse_explore', { query: 'lonelyLeaf' });
    const text = res.content[0].text;
