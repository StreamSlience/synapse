...
describe('git sync hooks', () => {
  ...
  beforeEach(() => {
    repo = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-githooks-'));
  });
  ...
  it('installs all default hooks, executable, invoking synapse sync', () => {
    gitInit(repo);
    ...
    for (const hook of DEFAULT_SYNC_HOOKS) {
      ...
      expect(body).toContain('synapse sync');
      expect(body).toContain('command -v synapse'); // no-op when not on PATH
  ...
  it('is idempotent — re-install does not duplicate the block', () => {
    ...
    const occurrences = body.split('# >>> synapse sync hook >>>').length - 1;
  ...
  it('remove keeps user content when the hook is shared', () => {
    ...
    expect(body).not.toContain('synapse sync');
  });
