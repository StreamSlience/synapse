/**
 * Regression coverage for issue #874: `synapse index` produced 0 nodes / 0
 * edges while `synapse init` worked, and appeared to wipe the graph.
 *
...
function runSynapse(args: string[], cwd: string): string {
  return execFileSync(process.execPath, [BIN, ...args], {
    cwd,
    encoding: 'utf-8',
    env: { ...process.env, SYNAPSE_NO_DAEMON: '1' },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
}

function graphCounts(dir: string): { nodes: number; edges: number } {
  const cg = Synapse.openSync(dir);
  try {
...
describe('synapse index — full re-index keeps the graph populated (#874)', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-index-cmd-'));
  ...
  it('--quiet path also rebuilds a populated graph', () => {
    ...
    runSynapse(['index', '--quiet'], tempDir);
    const afterIndex = graphCounts(tempDir);
