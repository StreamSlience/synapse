...
// A marker-delimited Synapse block exactly as a previous installer
...
const LEGACY_BLOCK = [
  ...
  '## Synapse',
  '',
  'Prefer `synapse_search` / `synapse_callers` over grep.',
  '<!-- SYNAPSE_END -->',
].join('\n');
        ...
        describe(`location=${location}`, () => {
          ...
          it('install preserves a pre-existing sibling MCP server (where applicable)', () => {
            ...
            if (target.id === 'opencode') {
              ...
              expect(after.mcp.synapse).toBeDefined();
            } else {
              expect(after.mcpServers.other).toBeDefined();
              expect(after.mcpServers.synapse).toBeDefined();
            }
          });
...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('codex: install writes config.toml AND the AGENTS.md synapse block (#704)', () => {
    ...
    expect(body).toContain('## Synapse');
    expect(body).toContain('synapse explore');
  ...
  it('codex: install replaces a legacy AGENTS.md synapse block with the current one, keeping user content', () => {
    const codex = getTarget('codex')!;
    ...
    expect(body).not.toContain('Prefer `synapse_search`');
    expect(body).toContain('synapse explore');
  ...
  it('opencode: prefers .jsonc when both .json and .jsonc exist', () => {
    ...
    expect(jsonText).not.toContain('synapse');
  });
  ...
  it('opencode: preserves line and block comments through install + idempotent re-run', () => {
    ...
    expect(afterInstall).toContain('"synapse"');
  ...
  it('opencode: install writes the AGENTS.md synapse block (#704)', () => {
    const opencode = getTarget('opencode')!;
    const result = opencode.install('global', { autoAllow: true });
    const agentsMd = path.join(tmpHome, '.config', 'opencode', 'AGENTS.md');
    expect(fs.existsSync(agentsMd)).toBe(true);
    expect(fs.readFileSync(agentsMd, 'utf-8')).toContain('synapse explore');
    expect(result.files.find((f) => f.path.endsWith('AGENTS.md'))?.action).toBe('created');
  });

  it('opencode: install replaces a legacy AGENTS.md synapse block, preserving user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
  ...
  it('opencode: uninstall strips a leftover synapse block from AGENTS.md, keeping user content', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const agentsMd = path.join(dir, 'AGENTS.md');
    ...
    expect(body).toContain('# My personal opencode instructions');
    expect(body).toContain('Always respond in pirate.');
    expect(body).not.toContain('SYNAPSE_START');
  });
  ...
  it('gemini: install writes settings.json (mcpServers.synapse) and the GEMINI.md block (#704)', () => {
    ...
    expect(fs.readFileSync(geminiMd, 'utf-8')).toContain('synapse explore');

    const cfg = JSON.parse(fs.readFileSync(settings, 'utf-8'));
    expect(cfg.mcpServers.synapse).toEqual({ type: 'stdio', command: 'synapse', args: ['serve', '--mcp'] });
  });
  ...
  it('gemini: uninstall strips synapse but leaves pre-existing settings (security.auth) intact', () => {
    const gemini = getTarget('gemini')!;
    const settings = path.join(tmpHome, '.gemini', 'settings.json');
    fs.mkdirSync(path.dirname(settings), { recursive: true });
    fs.writeFileSync(settings, JSON.stringify({
  ...
  it('gemini: uninstall strips a leftover GEMINI.md synapse block, keeping user content', () => {
    const gemini = getTarget('gemini')!;
    const geminiMd = path.join(tmpHome, '.gemini', 'GEMINI.md');
  ...
  it('kiro: install writes settings/mcp.json (mcpServers.synapse) and no steering doc (#529)', () => {
    ...
    const steering = path.join(tmpHome, '.kiro', 'steering', 'synapse.md');
  ...
  it('kiro: install deletes a leftover steering synapse.md (self-heal) (#529)', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'synapse.md');
  ...
  it('kiro: uninstall strips synapse but leaves sibling MCP servers intact', () => {
    const kiro = getTarget('kiro')!;
    const mcp = path.join(tmpHome, '.kiro', 'settings', 'mcp.json');
    fs.mkdirSync(path.dirname(mcp), { recursive: true });
    fs.writeFileSync(mcp, JSON.stringify({
    ...
    const after = JSON.parse(fs.readFileSync(mcp, 'utf-8'));
    expect(after.mcpServers.other).toBeDefined();
    expect(after.mcpServers.synapse).toBeUndefined();
  });

  it('kiro: uninstall removes a leftover steering synapse.md file outright', () => {
    const kiro = getTarget('kiro')!;
    const steering = path.join(tmpHome, '.kiro', 'steering', 'synapse.md');
    fs.mkdirSync(path.dirname(steering), { recursive: true });
    fs.writeFileSync(steering, `${LEGACY_BLOCK}\n`);
  ...
  it('kiro: uninstall removes our steering doc but leaves a sibling (product.md) untouched', () => {
    ...
    const ours = path.join(tmpHome, '.kiro', 'steering', 'synapse.md');
  ...
  it('kiro: local install writes ./.kiro/settings/mcp.json and no steering doc (#529)', () => {
    ...
    expect(paths.some((p) => p.endsWith('/.kiro/steering/synapse.md'))).toBe(false);
  });
  ...
  it('antigravity: install writes to LEGACY ~/.gemini/antigravity/mcp_config.json when no migration marker', () => {
    ...
    expect(cfg.mcpServers.synapse).toBeDefined();
  ...
  it('antigravity: entry has NO `type` field (Antigravity rejects entries with it)', () => {
    ...
    expect(cfg.mcpServers.synapse.type).toBeUndefined();
    expect(cfg.mcpServers.synapse.command).toBeDefined();
    expect(cfg.mcpServers.synapse.args).toEqual(['serve', '--mcp']);
  });

  it('antigravity: install migrates a legacy synapse entry to the unified path when marker appears', () => {
    const antigravity = getTarget('antigravity')!;
    // Simulate: user installed on the legacy path, then Antigravity
    // migrated their config (dropped the `.migrated` marker + created
    // the unified file). Re-running synapse install should land
    // synapse in the new file AND strip the stale legacy entry.
    const legacyFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(legacyFile), { recursive: true });
    fs.writeFileSync(legacyFile, JSON.stringify({
      mcpServers: { synapse: { command: 'synapse', args: ['serve', '--mcp'] } },
    }, null, 2) + '\n');
    fs.mkdirSync(path.join(tmpHome, '.gemini', 'config'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.gemini', 'config', '.migrated'), '');
    ...
    expect(unified.mcpServers.synapse).toBeDefined();
    // Legacy file's synapse entry got stripped.
  ...
  it('antigravity: uninstall removes only synapse, sibling MCP server survives', () => {
    const antigravity = getTarget('antigravity')!;
    const mcpFile = path.join(tmpHome, '.gemini', 'antigravity', 'mcp_config.json');
    fs.mkdirSync(path.dirname(mcpFile), { recursive: true });
    fs.writeFileSync(mcpFile, JSON.stringify({
  ...
  it('antigravity: uninstall sweeps BOTH legacy and unified paths (handles migration half-state)', () => {
    const antigravity = getTarget('antigravity')!;
    // User had synapse in BOTH files (e.g. legacy install + post-migration
  ...
  it('gemini + antigravity: both installed coexist (separate MCP files, shared GEMINI.md)', () => {
    ...
    expect(cliCfg.mcpServers.synapse).toBeDefined();
    expect(ideCfg.mcpServers.synapse).toBeDefined();
    ...
    expect(cliAfter.mcpServers.synapse).toBeDefined();
  });

  it('hermes: install adds synapse MCP server and cli toolset, preserving existing yaml', () => {
    ...
    expect(body).toContain('    - mcp-synapse');
  ...
  it('hermes: uninstall removes only synapse MCP server and toolset entry', () => {
    const hermes = getTarget('hermes')!;
    const config = path.join(tmpHome, '.hermes', 'config.yaml');
    fs.mkdirSync(path.dirname(config), { recursive: true });
    ...
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('synapse:');
    expect(body).not.toContain('mcp-synapse');
  ...
  // the next sibling key, truncated the cli block, and spliced `- mcp-synapse`
  ...
  it('hermes: install preserves PyYAML-default list-at-same-indent style (issue #456)', () => {
    ...
    // mcp-synapse appended at the same 2-space indent as existing items
    ...
    // start with `cli:` and not contain a stray 4-space `mcp-synapse`
  ...
  it('hermes: uninstall reverses the install on a PyYAML-default config', () => {
    ...
    expect(installed).toContain('- mcp-synapse');
    expect(installed).toContain('synapse:');

    hermes.uninstall('global');
    const body = fs.readFileSync(config, 'utf-8');
    expect(body).not.toContain('mcp-synapse');
    expect(body).not.toContain('command: synapse');
    expect(body).toContain('  cli:\n  - hermes-cli\n  - browser');
    expect(body).toContain('  telegram:\n  - hermes-telegram');
  });

  it('opencode: uninstall removes only mcp.synapse, preserves comments and siblings', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    const file = path.join(dir, 'opencode.jsonc');
    ...
    expect(afterUninstall).not.toContain('synapse');
    expect(afterUninstall).toContain('// important comment');
    expect(afterUninstall).toContain('"other"');
  });

  it('codex: user-added key inside [mcp_servers.synapse] survives idempotent re-install', () => {
    const codex = getTarget('codex')!;
  ...
  it('claude: install creates the CLAUDE.md synapse block (#704)', () => {
    const claude = getTarget('claude')!;
    const result = claude.install('local', { autoAllow: false });
  ...
  it('claude: install replaces a legacy CLAUDE.md synapse block, keeping user content', () => {
    const claude = getTarget('claude')!;
    const claudeMd = path.join(tmpCwd, '.claude', 'CLAUDE.md');
  ...
  it('claude: local install migrates a legacy ./.claude.json synapse entry into ./.mcp.json', () => {
    const claude = getTarget('claude')!;
    const legacy = path.join(tmpCwd, '.claude.json');
    fs.writeFileSync(
      legacy,
      JSON.stringify({ mcpServers: { synapse: { type: 'stdio', command: 'synapse', args: ['serve', '--mcp'] } } }, null, 2),
    );

    claude.install('local', { autoAllow: false });

    // synapse now lives in .mcp.json; the legacy file (which held only
    // synapse) is gone.
    const mcp = JSON.parse(fs.readFileSync(path.join(tmpCwd, '.mcp.json'), 'utf-8'));
    expect(mcp.mcpServers.synapse).toBeDefined();
  ...
  it('claude: legacy ./.claude.json migration preserves sibling servers and unrelated keys', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        mcpServers: {
          synapse: { type: 'stdio', command: 'synapse', args: ['serve', '--mcp'] },
          other: { command: 'x' },
        },
        somethingElse: true,
      }, null, 2),
    ...
    // Only synapse is stripped from the legacy file; siblings survive.
  ...
  it('claude: uninstall strips synapse from ./.mcp.json and a legacy ./.claude.json', () => {
    const claude = getTarget('claude')!;
    // A user left with both the working .mcp.json and a stale .claude.json.
    fs.writeFileSync(
      path.join(tmpCwd, '.mcp.json'),
      JSON.stringify({ mcpServers: { synapse: { command: 'synapse' } } }, null, 2),
    );
    fs.writeFileSync(
      path.join(tmpCwd, '.claude.json'),
      JSON.stringify({ mcpServers: { synapse: { command: 'synapse' }, other: { command: 'x' } } }, null, 2),
    );
    ...
    expect(legacy.mcpServers.synapse).toBeUndefined();
    expect(legacy.mcpServers.other).toBeDefined();
  });

  // ---- Legacy auto-sync hook cleanup ----
  // Pre-0.8 installs wrote `synapse mark-dirty` / `sync-if-dirty`
  ...
  function legacyHookSettings(): Record<string, any> {
    return {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'synapse mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'synapse sync-if-dirty' }] },
  ...
  it('claude: install strips stale synapse auto-sync hooks but keeps the user\'s GitKraken hook', () => {
    const claude = getTarget('claude')!;
    ...
    expect(stopCommands).not.toContain('synapse sync-if-dirty');
    ...
    expect(after.permissions?.allow).toContain('mcp__synapse__synapse_search');
  });
    ...
    const file = seedSettings('global', {
      hooks: {
        Stop: [
          {
            hooks: [
              { type: 'command', command: 'synapse sync-if-dirty' },
  ...
  it('claude: cleanupLegacyHooks is a byte-for-byte no-op without synapse hooks', () => {
    const original =
  ...
  it('claude: uninstall strips stale hooks written in the npx form (local)', () => {
    ...
    const file = seedSettings('local', {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'npx @colbymchenry/synapse mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'npx @colbymchenry/synapse sync-if-dirty' }] },
        ],
      },
    });
...
describe('Installer targets — TOML serializer (Codex backbone)', () => {
  it('builds a [mcp_servers.synapse] block with command + args', () => {
    const block = buildTomlTable('mcp_servers.synapse', {
      command: 'synapse',
      args: ['serve', '--mcp'],
    });
    expect(block).toContain('[mcp_servers.synapse]');
    expect(block).toContain('command = "synapse"');
    expect(block).toContain('args = ["serve", "--mcp"]');
  });

  it('upsert inserts into empty content', () => {
    const block = buildTomlTable('mcp_servers.synapse', { command: 'synapse', args: ['serve'] });
    const { content, action } = upsertTomlTable('', 'mcp_servers.synapse', block);
    expect(action).toBe('inserted');
    expect(content.startsWith('[mcp_servers.synapse]')).toBe(true);
  });

  it('upsert is idempotent — second call returns unchanged', () => {
    const block = buildTomlTable('mcp_servers.synapse', { command: 'synapse', args: ['serve'] });
    const first = upsertTomlTable('', 'mcp_servers.synapse', block);
    const second = upsertTomlTable(first.content, 'mcp_servers.synapse', block);
  ...
  it('upsert replaces an existing block in place, preserving sibling tables', () => {
    ...
    const newBlock = buildTomlTable('mcp_servers.synapse', {
      command: 'synapse',
      args: ['serve', '--mcp'],
    });
    const { content, action } = upsertTomlTable(existing, 'mcp_servers.synapse', newBlock);
    expect(action).toBe('replaced');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).toContain('[zzz]');
    expect(content).toContain('baz = "qux"');
    expect(content).toContain('command = "synapse"');
    expect(content).not.toContain('old-synapse');
  });
  ...
  it('removeTomlTable strips the block and preserves siblings', () => {
    ...
    const { content, action } = removeTomlTable(existing, 'mcp_servers.synapse');
    expect(action).toBe('removed');
    expect(content).toContain('[other_table]');
    expect(content).toContain('foo = "bar"');
    expect(content).not.toContain('mcp_servers.synapse');
  });
  ...
  it('upsert preserves an array-of-tables sibling [[foo]]', () => {
    ...
    const block = buildTomlTable('mcp_servers.synapse', { command: 'synapse', args: ['serve'] });
    const { content } = upsertTomlTable(existing, 'mcp_servers.synapse', block);
    expect(content.match(/\[\[foo\]\]/g)?.length).toBe(2);
    expect(content).toContain('[mcp_servers.synapse]');
  });
});

describe('Installer — uninstallTargets sweep (synapse uninstall)', () => {
  let tmpHome: string;
  let tmpCwd: string;
  let origCwd: string;
  let homeRestore: { restore: () => void };
...
describe('Installer — Cursor rules file cleanup on uninstall', () => {
  ...
  const rulesFile = () => path.join(process.cwd(), '.cursor', 'rules', 'synapse.mdc');
  ...
  it('uninstall deletes a leftover synapse.mdc entirely (no orphaned frontmatter left behind)', () => {
  ...
  it('install self-heals a leftover synapse.mdc (#529)', () => {
    plantLegacyRulesFile();
    const result = cursor.install('local', { autoAllow: true });
    expect(fs.existsSync(rulesFile())).toBe(false);
    expect(result.files.some((f) => f.path.endsWith('synapse.mdc') && f.action === 'removed')).toBe(true);
  });

  it('uninstall preserves user content added outside the synapse markers (strips only our block)', () => {
    ...
    expect(after).not.toContain('synapse_search');
    expect(after).not.toContain('SYNAPSE_START');
  });
});
...
describe('Installer targets — opencode XDG config path (#535)', () => {
  ...
  it('greenfield: targets ~/.config/opencode even when the dir does not exist yet (#535)', () => {
    ...
    // ~/.config/opencode itself on first run; installing synapse FIRST
  ...
  it('install self-heals a pre-#535 %APPDATA% entry, preserving siblings and comments', () => {
    // A previous synapse version wrote into %APPDATA%/opencode. The user
    ...
    fs.writeFileSync(path.join(legacyDir(), 'opencode.jsonc'), [
      ...
      '  "$schema": "https://opencode.ai/config.json",',
      '  "mcp": {',
      '    "synapse": { "type": "local", "command": ["synapse", "serve", "--mcp"], "enabled": true },',
    ...
    expect(legacyText).not.toContain('synapse');
  ...
  it('uninstall sweeps the legacy %APPDATA% entry too (no prior re-install needed)', () => {
    // A user on the broken version goes straight to `synapse uninstall`:
    ...
    expect(fs.readFileSync(path.join(legacyDir(), 'opencode.json'), 'utf-8')).not.toContain('synapse');
