...
describe('installMainThreadWatchdog opt-out', () => {
  it('returns null (spawns nothing) when SYNAPSE_NO_WATCHDOG is set', () => {
    const prev = process.env.SYNAPSE_NO_WATCHDOG;
    process.env.SYNAPSE_NO_WATCHDOG = '1';
    try {
      expect(installMainThreadWatchdog()).toBeNull();
    } finally {
      if (prev === undefined) delete process.env.SYNAPSE_NO_WATCHDOG;
      else process.env.SYNAPSE_NO_WATCHDOG = prev;
    }
  });
});
...
describe('liveness watchdog (spawned, real watchdog process)', () => {
  ...
  it('SIGKILLs a process whose main thread wedges in a sync loop', async () => {
    const r = await runChild(
      { SYNAPSE_WATCHDOG_TIMEOUT_MS: '500' },
  ...
  it('does NOT kill a wedged process when SYNAPSE_NO_WATCHDOG=1', async () => {
    const { code, signal } = await runChild(
      { SYNAPSE_WATCHDOG_TIMEOUT_MS: '500', SYNAPSE_NO_WATCHDOG: '1' },
