 ...
 *   - A stale lockfile (dead pid) is cleared; `SYNAPSE_NO_DAEMON=1` opts out;
 ...
 *
 * These tests intentionally spawn real `node dist/bin/synapse.js` processes
 * over real sockets/pipes — the same surface a Claude Code / Cursor / Codex
 * install exercises. The daemon logs to `.synapse/daemon.log` (it has no
...
const BIN = path.resolve(__dirname, '../dist/bin/synapse.js');
...
function spawnServer(cwd: string, env: NodeJS.ProcessEnv = {}): SpawnedServer {
  const child = spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // harness into it (SYNAPSE_MCP_LOG_ATTACH=1) so the attach assertions
    // below can still observe a successful attach. A per-test env still wins.
    env: { SYNAPSE_MCP_LOG_ATTACH: '1', ...process.env, ...env },
...
function readLockPid(root: string): number | null {
  try {
    const raw = fs.readFileSync(path.join(root, '.synapse', 'daemon.pid'), 'utf8');
...
function readDaemonLog(root: string): string {
  try { return fs.readFileSync(path.join(root, '.synapse', 'daemon.log'), 'utf8'); }
  catch { return ''; }
}

function countListeningLines(root: string): number {
  return readDaemonLog(root).split('\n').filter((l) => l.includes('[Synapse daemon] Listening on')).length;
}
...
describe('Shared MCP daemon (issue #411)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-mcp-daemon-'));
    const cg = await Synapse.init(tempDir);
    cg.close();
  ...
  it('two invocations share ONE detached daemon; both attach as proxies', async () => {
    const env = { SYNAPSE_DAEMON_IDLE_TIMEOUT_MS: '15000' };
    ...
    expect(firstResp.result.serverInfo.name).toBe('synapse');
    ...
    await waitFor(() => fs.existsSync(path.join(realRoot, '.synapse', 'daemon.pid')), 8000);
    ...
    expect(secondResp.result.serverInfo.name).toBe('synapse');
  ...
  it('concurrent launchers converge on a single daemon (lockfile race — must-fix 1)', async () => {
    ...
    for (let i = 0; i < procs.length; i++) {
      ...
      expect(resp.result.serverInfo.name).toBe('synapse');
    }
  ...
  it('daemon survives the first client dying; a second client keeps working (must-fix 2 / #277)', async () => {
    ...
    const env = { SYNAPSE_DAEMON_IDLE_TIMEOUT_MS: '30000', SYNAPSE_PPID_POLL_MS: '200' };
  ...
  it('SYNAPSE_NO_DAEMON=1 keeps each process independent (no socket/pidfile)', async () => {
    const env = { SYNAPSE_NO_DAEMON: '1' };
    const first = spawnServer(tempDir, env);
    servers.push(first);
    sendInitialize(first.child, `file://${tempDir}`, 1);
    await waitFor(() => findResponse(first.stdout, 1), 10000);
    ...
    expect(fs.existsSync(path.join(realRoot, '.synapse', 'daemon.pid'))).toBe(false);
    expect(fs.existsSync(path.join(realRoot, '.synapse', 'daemon.log'))).toBe(false);
  }, 20000);
  ...
  it('clears a stale (dead-pid) lockfile and a fresh daemon takes over', async () => {
    ...
    fs.writeFileSync(
      path.join(realRoot, '.synapse', 'daemon.pid'),
      JSON.stringify({
        pid: 999_999,
        version: '0.0.0-fake',
  ...
  it('proxy falls back to direct mode on a daemon version mismatch', async () => {
    ...
    const miniServer = net.createServer((sock) => {
      sock.write(JSON.stringify({ synapse: '0.0.0-mismatch', pid: 1, socketPath: sockPath, protocol: 1 }) + '\n');
    });
  ...
  it('exits on the inactivity backstop even while a client stays connected (#692)', async () => {
    ...
    const env = { SYNAPSE_DAEMON_MAX_IDLE_MS: '1500', SYNAPSE_DAEMON_IDLE_TIMEOUT_MS: '60000' };
    const server = spawnServer(tempDir, env);
    servers.push(server);
    sendInitialize(server.child, `file://${tempDir}`, 1);
  ...
  it('daemon idle-times-out after the last client disconnects', async () => {
    const env = { SYNAPSE_DAEMON_IDLE_TIMEOUT_MS: '800', SYNAPSE_PPID_POLL_MS: '200' };
    const server = spawnServer(tempDir, env);
    servers.push(server);
    sendInitialize(server.child, `file://${tempDir}`, 1);
    await waitFor(() => findResponse(server.stdout, 1), 10000);
  ...
  it('proxy survives the daemon dying mid-session and keeps serving (#662)', async () => {
    ...
    // is live. The proxy must NOT exit (losing Synapse for that session) — it
    // falls back to an in-process engine and keeps answering.
    const env = { SYNAPSE_DAEMON_IDLE_TIMEOUT_MS: '30000', SYNAPSE_PPID_POLL_MS: '5000' };
    const server = spawnServer(tempDir, env);
    servers.push(server);
    sendInitialize(server.child, `file://${tempDir}`, 1);
    await waitFor(() => findResponse(server.stdout, 1), 10000);
    ...
    sendMessage(server.child, { jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'synapse_status', arguments: {} } });
    ...
    sendMessage(server.child, { jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'synapse_status', arguments: {} } });
