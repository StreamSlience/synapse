/**
 * synapse_files path-filter normalization (#426)
 *
...
describe('synapse_files path normalization', () => {
  let tempDir: string;
  let cg: Synapse;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('synapse_files', {
