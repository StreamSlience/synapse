 ...
 * the MCP server was blocking the initialize response on Synapse.open() and
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // daemon (logging to .synapse/daemon.log, not the child's stderr); the
...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  it('responds to initialize quickly when no .synapse exists in cwd', async () => {
  ...
  it('sends initialize response BEFORE tryInitializeDefault finishes', async () => {
    // Seed a real .synapse so the server's tryInitializeDefault path runs
    // its full body: Synapse.open() (which awaits initGrammars()) and then
    ...
    const json = JSON.parse(response.text);
    expect(json.id).toBe(0);
    expect(json.result.serverInfo.name).toBe('synapse');
  }, 20000);
