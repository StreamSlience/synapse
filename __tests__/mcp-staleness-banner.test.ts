 ...
 * the full real path: real Synapse index + real ToolHandler.execute().
 *
...
describe('MCP staleness banner', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-stale-banner-'));
    ...
    // files shared imports/calls, synapse_search responses would surface
  ...
  it('prepends a stale banner when the response references a pending file', async () => {
    ...
    const res = await handler.execute('synapse_search', { query: 'alphaOnly' });
  ...
  it('lists pending files under "Pending sync" in synapse_status', async () => {
    cg.watch({ debounceMs: 4000, inertForTests: true });
    await cg.waitUntilWatcherReady();
  ...
  it('surfaces the degraded state as its own section in synapse_status (#876)', async () => {
    degradeWatcher();
