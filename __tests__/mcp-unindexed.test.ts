 ...
 *
 * An MCP session attached to a workspace with no .synapse/ must go quiet
 ...
 * isError responses teach an agent to abandon synapse for the whole
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // anyway (the daemon socket lives in .synapse/), and this keeps the
    // suite from leaking a detached daemon in the indexed test.
    // SYNAPSE_WASM_RELAUNCHED skips the --liftoff-only re-exec: without
    ...
    env: { ...process.env, SYNAPSE_NO_DAEMON: '1', SYNAPSE_WASM_RELAUNCHED: '1' },
...
describe('Unindexed-workspace session policy', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-unindexed-'));
  });
  ...
  it('initialize returns the short "inactive" instructions, not the playbook', async () => {
    ...
    expect(instructions).toMatch(/synapse init/);
    ...
    expect(instructions).not.toMatch(/synapse_explore/);
  });
  ...
  it('an INDEXED workspace still gets the full playbook and all tools', async () => {
    ...
    const cg = await Synapse.init(tempDir, { index: true });
    cg.close();
    ...
    expect(tools.map((t) => t.name)).toContain('synapse_explore');
  });
});
...
describe('No-error policy on expected conditions', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-noerror-'));
  });
  ...
  it('cross-project query to an unindexed path is SUCCESS-shaped guidance, not isError', async () => {
    const res = await new ToolHandler(null).execute('synapse_search', {
      query: 'anything',
      projectPath: tempDir,
    });
    ...
    expect(res.content[0]!.text).toMatch(/synapse init/);
  ...
  it('no-default-project (working-directory detection miss) is SUCCESS-shaped guidance', async () => {
    const res = await new ToolHandler(null).execute('synapse_search', { query: 'anything' });

    expect(res.isError).toBeUndefined();
    expect(res.content[0]!.text).toMatch(/No Synapse project is loaded/);
...
describe('search kind filter', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-kind-'));
    fs.writeFileSync(
      path.join(tempDir, 'types.ts'),
      'export type PaymentMethod = { id: string };\nexport function pay(): void {}\n'
    );
    cg = await Synapse.init(tempDir, { index: true });
  });
  ...
  it("kind: 'type' (the advertised enum value) finds type aliases", async () => {
    const res = await new ToolHandler(cg).execute('synapse_search', {
      query: 'PaymentMethod',
      kind: 'type',
    });
