 ...
 * SYNAPSE_DOWNLOAD_BASE — no real network, no published release needed. The
...
const asset = `synapse-${target}.tar.gz`;
...
// A temp dir standing in for the installed @colbymchenry/synapse main package.
function makePkg(version = '9.9.9-test'): string {
  const dir = mkTmp('pkg');
  fs.copyFileSync(SHIM_SRC, path.join(dir, 'npm-shim.js'));
  fs.writeFileSync(path.join(dir, 'package.json'),
    JSON.stringify({ name: '@colbymchenry/synapse', version }) + '\n');
  return dir;
}
...
function writeLauncher(binDir: string): void {
  ...
  const p = path.join(binDir, 'synapse');
...
describe.skipIf(isWindows)('npm-shim launcher', () => {
  it('runs the installed optional-dependency bundle without any download', async () => {
    const pkg = makePkg();
    const platformPkg = path.join(pkg, 'node_modules', '@colbymchenry', `synapse-${target}`);
    writeLauncher(path.join(platformPkg, 'bin'));
    fs.writeFileSync(path.join(platformPkg, 'package.json'),
      JSON.stringify({ name: `@colbymchenry/synapse-${target}`, version: '9.9.9-test' }) + '\n');
    const cache = mkTmp('cache');
    const r = await runShim(pkg, ['--probe-abc'], { SYNAPSE_INSTALL_DIR: cache });
  ...
  it('uses an already-cached bundle even when downloads are disabled', async () => {
    ...
    const r = await runShim(pkg, ['--probe-xyz'], {
      SYNAPSE_INSTALL_DIR: cache,
      SYNAPSE_NO_DOWNLOAD: '1',
    });
  ...
  it('prints actionable guidance and exits 1 when disabled with no bundle', async () => {
    ...
    const r = await runShim(pkg, ['--version'], {
      SYNAPSE_INSTALL_DIR: mkTmp('cache'),
      SYNAPSE_NO_DOWNLOAD: '1',
    });

    expect(r.status).toBe(1);
    expect(r.stderr).toContain(`no prebuilt bundle for ${target}`);
    expect(r.stderr).toContain(`@colbymchenry/synapse-${target}`);
...
describe.skipIf(!CAN_NET)('npm-shim download fallback (local HTTPS)', () => {
  ...
  beforeAll(async () => {
    ...
    // Build a fake bundle archive (synapse-<target>/bin/synapse), like a real release asset.
    const work = mkTmp('fixture');
    writeLauncher(path.join(work, `synapse-${target}`, 'bin'));
    const archive = path.join(work, asset);
    execSync(`tar -czf ${JSON.stringify(archive)} -C ${JSON.stringify(work)} synapse-${target}`);
  ...
  function netEnv(cache: string): Record<string, string> {
    return {
      SYNAPSE_INSTALL_DIR: cache,
      SYNAPSE_DOWNLOAD_BASE: `https://127.0.0.1:${port}`,
  ...
  it('downloads, verifies the checksum, extracts, and execs the bundle', async () => {
    ...
    expect(fs.existsSync(path.join(cache, 'bundles', `${target}-5.0.0-net`, 'bin', 'synapse'))).toBe(true);
  }, 20000);
