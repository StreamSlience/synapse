...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use Synapse integration)
// =============================================================================
...
describe('Graph Traversal Both Direction', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should traverse both directions from a node', async () => {
    const Synapse = (await import('../src/index')).default;
...
`);
    ...
    const cg = Synapse.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'synapse.db');
...
`);
      ...
      const cg = Synapse.initSync(tmpDir, {
        config: { include: ['src/**/*.ts'], exclude: [] },
      });
      await cg.indexAll();
...
`);
      ...
      // note) so synapse_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via Synapse.uninitialize()', async () => {
    const Synapse = (await import('../src/index')).default;

    // Initialize
    const cg = Synapse.initSync(testDir);
    expect(Synapse.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .synapse directory should be removed
    expect(Synapse.isInitialized(testDir)).toBe(false);
  });
});
