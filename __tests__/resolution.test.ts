...
describe('Resolution Module', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-resolution-test-'));
  });
...
`;
  ...
  describe('Integration Tests', () => {
    it('should create resolver from Synapse instance', async () => {
...
`
    ...
    it('TS type_alias object-shape members resolve method calls (#359)', async () => {
      ...
      fs.mkdirSync(path.join(tempDir, 'synapse'));
...
`
      ...
      fs.writeFileSync(
        path.join(tempDir, 'synapse', 'stdio-client.ts'),
...
#include "utils/helpers.hpp"`;
    ...
    it('should discover include directories from compile_commands.json', () => {
      ...
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-cpp-test-'));
      try {
        const compileDb = [
          {
    ...
    it('connects #include to the real header file via include-dir scan (end-to-end)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-cpp-e2e-'));
      try {
        fs.mkdirSync(path.join(tempProject, 'include'), { recursive: true });
        fs.mkdirSync(path.join(tempProject, 'src'), { recursive: true });
        fs.writeFileSync(
        ...
        clearCppIncludeDirCache();
        cg = await Synapse.init(tempProject, { index: true });
        ...
        const db = DatabaseConnection.open(path.join(tempProject, '.synapse', 'synapse.db'));
  ...
  describe('PHP Include Resolution', () => {
    ...
    it('resolves require_once to a file→file imports edge (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-php-e2e-'));
      try {
        fs.mkdirSync(path.join(tempProject, 'src'), { recursive: true });
        fs.writeFileSync(
    ...
    it('resolves a subdirectory include path to the correct file (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-php-subdir-'));
      try {
    ...
    it('does not mis-connect an unresolvable include to a same-named file elsewhere (#660)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-php-misresolve-'));
      try {
