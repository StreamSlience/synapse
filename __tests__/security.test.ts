...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-security-test-'));
}
...
describe('Symlink escape prevention (#527)', () => {
  ...
  // is the chokepoint both content-serving read sinks go through (synapse_node
  // includeCode + synapse_explore source rendering), so it must resolve
  ...
  it('end-to-end: getCode never serves an out-of-root file reached via a dir symlink', async () => {
    ...
    const cg = Synapse.initSync(root, { config: { include: ['**/*.ts'], exclude: [] } });
    try {
      await cg.indexAll();
...
describe('MCP Input Validation', () => {
  ...
  it('should reject non-string query in synapse_search', async () => {
    const result = await handler.execute('synapse_search', { query: null });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject empty string query in synapse_search', async () => {
    const result = await handler.execute('synapse_search', { query: '' });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should accept valid query in synapse_search', async () => {
    const result = await handler.execute('synapse_search', { query: 'example' });
    expect(result.isError).toBeFalsy();
  });

  it('should clamp limit to valid range in synapse_search', async () => {
    // Extremely large limit should still work (clamped to 100)
    const result = await handler.execute('synapse_search', { query: 'example', limit: 999999 });
    expect(result.isError).toBeFalsy();
  });

  it('should reject non-string symbol in synapse_callers', async () => {
    const result = await handler.execute('synapse_callers', { symbol: 123 });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject non-string query in synapse_explore', async () => {
    const result = await handler.execute('synapse_explore', { query: undefined });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should truncate oversized tool output', async () => {
    // Force a huge result set through synapse_search; the response must be
    ...
    const fakeHandler = new ToolHandler(fakeCg as unknown as Synapse);

    const result = await fakeHandler.execute('synapse_search', { query: 'x' });

    expect(result.isError).toBeFalsy();
    expect(result.content[0].text).toContain('... (output truncated)');
  });

  it('should reject non-string symbol in synapse_impact', async () => {
    const result = await handler.execute('synapse_impact', { symbol: [] });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in synapse_node', async () => {
    const result = await handler.execute('synapse_node', { symbol: false });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in synapse_callees', async () => {
    const result = await handler.execute('synapse_callees', { symbol: {} });
    expect(result.isError).toBe(true);
  });

  it('should handle NaN limit gracefully', async () => {
    const result = await handler.execute('synapse_search', { query: 'example', limit: 'abc' });
    expect(result.isError).toBeFalsy();
  });

  it('should handle negative limit gracefully', async () => {
    const result = await handler.execute('synapse_search', { query: 'example', limit: -5 });
    expect(result.isError).toBeFalsy();
  });

  // #230: getSynapse must reject a sensitive system directory passed as
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      const result = await handler.execute('synapse_search', {
        query: 'example',
        projectPath: '/etc',
      });
      expect(result.isError).toBe(true);
