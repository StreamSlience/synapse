/**
 * Tests for the CI/scripting fields `synapse status --json` exposes (issue
 * #329): the `version`, `indexPath`, and `lastIndexed` fields, plus the
 * matching `Synapse.getLastIndexedAt()` library method.
 *
...
describe('synapse status --json — CI fields (#329)', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-status-json-'));
  });
  afterEach(() => {
  ...
  it('status --json on an UNINITIALIZED project reports version + indexPath + lastIndexed:null', () => {
    ...
    expect(out.indexPath as string).toContain('.synapse');
