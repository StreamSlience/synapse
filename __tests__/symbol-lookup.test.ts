 ...
 *
 * Pinned because the lookup vocabulary is what makes synapse useful
...
function tmpRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-symbol-lookup-'));
}
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — module-qualified lookups (#173)', () => {
  ...
  beforeEach(async () => {
    ...
    cg = Synapse.initSync(projectRoot, {
  ...
  it('synapse_node with a `file` hint pins an overloaded name to that file', async () => {
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — dotted lookups (regression for #173 fix)', () => {
  ...
  it('synapse_node on an ambiguous bare name returns ALL overloads with bodies (no guess)', async () => {
    ...
    const res = await handler.execute('synapse_node', { symbol: 'request', includeCode: true });
    const text = res.content?.[0]?.text ?? '';
