...
describe('detectInstallMethod', () => {
  ...
  it('detects a unix bundle and derives the install dir from the versions/ layout', () => {
    const root = '/home/u/.synapse/versions/v0.9.9';
    const filename = `${root}/lib/dist/bin/synapse.js`;
    const present = new Set([`${root}/node`, `${root}/bin/synapse`, '/home/u/.synapse']);
    ...
    expect(m).toEqual({
      ...
      os: 'unix',
      bundleRoot: root,
      installDir: '/home/u/.synapse',
    });
  });

  it('detects a windows bundle and derives the install dir from current\\', () => {
    const root = 'C:/Users/u/AppData/Local/synapse/current';
    const filename = `${root}/lib/dist/bin/synapse.js`;
    const present = new Set([`${root}/node.exe`, `${root}/bin/synapse.cmd`]);
    const m = detectInstallMethod({
      filename,
      platform: 'win32',
    ...
    expect(m.installDir?.replace(/\\/g, '/')).toBe('C:/Users/u/AppData/Local/synapse');
  });

  it('detects a global npm install', () => {
    const filename = '/usr/local/lib/node_modules/@colbymchenry/synapse/dist/bin/synapse.js';
    const m = detectInstallMethod({
      filename,
      platform: 'linux',
      cwd: '/home/u/project',
  ...
  it('detects a local (project) npm install as local', () => {
    ...
    const filename = `${cwd}/node_modules/@colbymchenry/synapse/dist/bin/synapse.js`;
  ...
  it('detects an npx run from the _npx cache', () => {
    const filename = '/home/u/.npm/_npx/abc123/node_modules/@colbymchenry/synapse/dist/bin/synapse.js';
  ...
  it('detects a source checkout via sibling package.json + .git', () => {
    const repo = '/home/u/dev/synapse';
    const filename = `${repo}/dist/bin/synapse.js`;
  ...
  it('returns unknown for an unrecognized layout', () => {
    const m = detectInstallMethod({
      filename: '/opt/weird/place/synapse.js',
      platform: 'linux',
      cwd: '/tmp',
      exists: () => false,
    });
...
describe('deriveInstallDir', () => {
  it('unix: returns the dir above versions/', () => {
    expect(deriveInstallDir('/a/b/.synapse/versions/v1.2.3', 'unix', () => true)).toBe('/a/b/.synapse');
  });
  ...
  it('windows: returns the parent of current\\', () => {
    expect(deriveInstallDir('C:/x/synapse/current', 'windows', () => true)?.replace(/\\/g, '/')).toBe('C:/x/synapse');
  });
  it('windows: null when basename is not current', () => {
    expect(deriveInstallDir('C:/x/synapse/v1', 'windows', () => true)).toBeNull();
  });
});
...
describe('version helpers', () => {
  ...
  it('parseLatestTagFromLocation extracts the tag from a releases redirect', () => {
    expect(parseLatestTagFromLocation('https://github.com/colbymchenry/synapse/releases/tag/v0.9.9')).toBe('v0.9.9');
  ...
  it('reindexAdvisory mentions the refresh commands', () => {
    ...
    expect(a).toContain('synapse sync');
    expect(a).toContain('synapse index -f');
  });
  ...
  it('buildWindowsUpgradeScript targets the right asset per arch and renames-not-deletes the exe', () => {
    ...
    expect(arm).toContain('releases/download/v1.2.3/synapse-win32-arm64.zip');
    ...
    expect(x64).toContain('synapse-win32-x64.zip');
  });
});
...
describe('runUpgrade', () => {
  ...
  it('unix bundle: runs the installer via sh with the derived install dir', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.synapse/versions/v0.9.8', installDir: '/h/.synapse' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
    ...
    expect(calls.runs[0].env?.SYNAPSE_INSTALL_DIR).toBe('/h/.synapse');
    expect(calls.logs.join('\n')).toMatch(/synapse sync/); // re-index advisory printed
  });

  it('unix bundle: falls back to wget, and errors when neither downloader exists', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'unix', bundleRoot: '/h/.synapse/versions/v0.9.8', installDir: null },
      currentVersion: '0.9.8',
      hasCommand: () => false,
    });
    const code = await runUpgrade({}, deps);
  ...
  it('windows bundle: runs a synchronous in-place (rename + extract) powershell upgrade', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'bundle', os: 'windows', bundleRoot: 'C:/x/synapse/current', installDir: 'C:/x/synapse' },
      currentVersion: '0.9.8',
      platform: 'win32',
    });
    const code = await runUpgrade({}, deps);
    ...
    expect(decoded).toContain('releases/download/v0.9.9/synapse-win32-');
  ...
  it('source: tells the user to git pull, runs nothing', async () => {
    const { deps, calls } = makeDeps({
      method: { kind: 'source', root: '/dev/synapse' },
      currentVersion: '0.9.8',
    });
    const code = await runUpgrade({}, deps);
    expect(code).toBe(0);
...
describe('index extraction-version stamp / isIndexStale', () => {
  ...
  it('stamps the current extraction version on full index and is not stale', async () => {
    ...
    const cg = await Synapse.init(dir, { index: false });
