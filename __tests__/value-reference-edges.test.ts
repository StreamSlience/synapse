 ...
 * "change this constant, affect its readers". Default on; SYNAPSE_VALUE_REFS=0
...
function valueRefReaders(cg: Synapse, constName: string): string[] {
...
describe('value-reference edges', () => {
  let dir: string;
  let cg: Synapse | undefined;

  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'synapse-valueref-'));
  });
  afterEach(() => {
    cg?.destroy();
    cg = undefined;
  ...
  function index(): Synapse {
    const g = Synapse.initSync(dir, { config: { include: ['**/*.ts', '**/*.tsx'], exclude: [] } });
    return g;
  }
  ...
  it('emits nothing when SYNAPSE_VALUE_REFS=0', async () => {
    const prev = process.env.SYNAPSE_VALUE_REFS;
    process.env.SYNAPSE_VALUE_REFS = '0';
    try {
      fs.writeFileSync(
        path.join(dir, 'config.ts'),
      ...
      if (prev === undefined) delete process.env.SYNAPSE_VALUE_REFS;
      else process.env.SYNAPSE_VALUE_REFS = prev;
    }
  });
});
