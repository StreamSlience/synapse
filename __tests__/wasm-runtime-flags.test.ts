 ...
 * (`Fatal process out of memory: Zone`) that crashed `synapse index` on large
...
describe('buildRelaunchArgv', () => {
  it('places the wasm flags first, then the script and its args', () => {
    expect(buildRelaunchArgv('/x/synapse.js', ['index', '/repo'], [])).toEqual([
      '--liftoff-only',
      '/x/synapse.js',
      'index',
      '/repo',
  ...
  it('preserves other existing node flags without duplicating ours', () => {
    expect(
      buildRelaunchArgv('/x/synapse.js', ['status'], ['--liftoff-only', '--enable-source-maps'])
    ).toEqual(['--liftoff-only', '--enable-source-maps', '/x/synapse.js', 'status']);
  });
