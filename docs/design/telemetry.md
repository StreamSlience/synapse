...
`synapse telemetry` CLI, MCP + installer wiring, `TELEMETRY.md`. Pending: Worker deploy
+ DNS, release.
Scope: public `synapse` engine (CLI + MCP server + installer)

Synapse is a local-first tool whose whole pitch is "your code never leaves your machine."
...
- How many machines actively use synapse (daily/weekly), and how does that change?
...
- No telemetry from the `synapse-pro` fork (see "synapse-pro rule" below).
...
4. **First-party endpoint.** Clients only ever talk to `telemetry.getsynapse.com`. The URL
...
| `synapse_version` | `0.9.12` | from package.json |
...
- **`usage_rollup`** — the workhorse. One event per `(day, kind, name)` per machine,
  ...
  (e.g. `synapse_explore`, `affected`), `count`, `error_count`, and for MCP:
...
2. `SYNAPSE_TELEMETRY=0|1` → forced off/on for that process
3. Global config `~/.synapse/telemetry.json` → stored user choice
...
- **Headless paths** (`npx synapse init`, MCP server — no TTY, never prompt): right
...
- **CLI:** `synapse telemetry status|on|off` (status prints the machine ID, current
  state, and what decided it). Deleting `~/.synapse/telemetry.json` resets everything,
...
(`~/.synapse/` is new — today nothing global exists. Coexists by filename if a user ever
indexes `$HOME` itself, since per-project data lives in `<project>/.synapse/` with fixed
other filenames.)
...
- **Buffer** — counters persist (debounced, async) to `~/.synapse/telemetry-queue.jsonl`.
...
- **Flush** — many CLI actions end via `process.exit()`, where `beforeExit` never fires
  ...
  `https://telemetry.getsynapse.com/v1/events` with `AbortSignal.timeout(1500)`,
  ...
  sender's claim merges back after an hour). `SYNAPSE_TELEMETRY_DEBUG=1` echoes
...
`telemetry.getsynapse.com` → small Worker living at `telemetry-worker/` in this repo —
...
## synapse-pro rule (do not lose this in upstream merges)

The private `synapse-pro` fork ships inside customer containers whose guarantee is
...
container sets `SYNAPSE_TELEMETRY=0` as belt-and-braces. This rule lives in the fork's
...
2. Worker + DNS live first (so the first shipping client never 404s), PostHog dashboards:
   ...
   languages indexed.
3. Client module + config + `synapse telemetry` subcommand + MCP `clientInfo` plumbing.
