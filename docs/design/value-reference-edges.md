...
**Status:** SHIPPED (default-on for TS/JS/tsx + Go + Python + Rust + Ruby + C + Java + C# + PHP + Scala + Kotlin + Swift + Dart + Pascal; `SYNAPSE_VALUE_REFS=0` disables). The
...
flow straight into `getImpactRadius` / `synapse impact` and the impact trail in
`synapse_explore` / `synapse_node` — no agent-behaviour change required.
...
Method per repo: index the same tree twice (value-refs on vs `SYNAPSE_VALUE_REFS=0`),
diff node/edge counts, spot-check precision, and measure `synapse impact` on a few
...
- **Impact API (the win):** `impact` ON vs OFF — `tablerIconProps` 1→170,
  `COLOR_PALETTE` 15→26, `CaptureUpdateAction` 61→86. This is what `synapse impact`
  and Synapse Pro's verdict engine consume via `getImpactRadius`.
- **Agent read-displacement: none — and that's expected.** On an indexed repo the agent
  answers impact questions in one synapse call (0 Read / 0 Grep in *both* arms), and it
  reaches for `synapse_search` / `callers`, **not** `impact`/`explore`, so it often
