# Synapse standalone installer for Windows (PowerShell).
#
...
#
# Upgrade with `synapse upgrade` (or just re-run this). To uninstall: remove
# $env:LOCALAPPDATA\synapse and drop its \current\bin entry from your user PATH.
#
# Environment:
...
$version = $env:SYNAPSE_VERSION
if (-not $version) {
  $version = (Invoke-RestMethod "https://api.github.com/repos/$repo/releases/latest").tag_name
}
if (-not $version) { throw "synapse: could not resolve latest version; set SYNAPSE_VERSION." }

# 3. Download + extract the bundle into a stable 'current' dir (overwritten on upgrade).
$url = "https://github.com/$repo/releases/download/$version/synapse-$target.zip"
Write-Host "Installing Synapse $version ($target)..."
...
# Archives contain a top-level synapse-<target>\ dir; flatten it.
$inner = Join-Path $dest "synapse-$target"
if (Test-Path $inner) {
