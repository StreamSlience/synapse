  ...
  node -e '
    ...
    for(const l of lines){try{const o=JSON.parse(l);
      ...
      for(const b of (Array.isArray(blocks)?blocks:[])){
        ...
        if(b.name==="Read")read++;
        else if(b.name==="mcp__synapse__synapse_node"){ if(b.input&&b.input.symbol)cgSym++; else cgFile++; }
        else if(/mcp__synapse__/.test(b.name))cgOther++;
      }
    }catch{}}
    console.log(`    Read=${read}  synapse_node[file]=${cgFile}  synapse_node[symbol]=${cgSym}  other_cg=${cgOther}  (cg exposed=${exposed})`);
  ' "$1"
}
...
echo "== NEW build (HEAD: synapse_node has Read parity) =="
...
echo "###### DONE — compare [new] vs [baseline]: does synapse_node[file] rise / Read fall? Logs: $OUT"
