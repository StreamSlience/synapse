#!/usr/bin/env bash
# A/B the PreToolUse(Read) REDIRECT hook (P1): does steering Read → synapse_node
# file-view actually move the agent off Read during implementation? BOTH arms use
# the CURRENT build with synapse attached and pre-warmed; the only difference is
...
#
#   arm [nohook] — synapse on, no hook   (does the better file-view get picked on its own?)
#   arm [hook]   — synapse on, + redirect hook   (does routing close it?)
#
# Reliable attach (works nested): each arm pre-warms a persistent daemon and skips
# the startup re-exec (SYNAPSE_WASM_RELAUNCHED=1), so claude connects before the
# agent's first turn. Judge by ACTUAL synapse usage in parse-run.mjs's "by type",
...
#   <indexed-repo>  a repo with a .synapse index (copied per arm; never mutated)
