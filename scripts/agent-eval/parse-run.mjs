...
for (const line of lines) {
  ...
  if (ev.type === 'system' && ev.subtype === 'init') {
    initTools = (ev.tools || []).filter(t => /synapse/.test(t));
  }
  if (ev.type === 'assistant' && ev.message?.content) {
    for (const block of ev.message.content) {
      if (block.type === 'tool_use') {
        ...
        else if (/synapse/.test(block.name)) detail = ` ${JSON.stringify(block.input?.query ?? block.input?.task ?? block.input?.symbol ?? '').slice(0,60)}`;
...
console.log(`\n=== ${file.split('/').pop()} ===`);
console.log(`synapse tools exposed: ${initTools ? initTools.length : '?'}`);
