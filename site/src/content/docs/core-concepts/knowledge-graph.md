...
Synapse stores three things: **nodes** (symbols and files), **edges** (relationships between them), and **files**. Every node and edge carries an exact `kind`, drawn from a fixed vocabulary so queries are consistent across languages.
...
See the [CLI](/synapse/reference/cli/) and [MCP Server](/synapse/reference/mcp-server/) references for how to run these.
