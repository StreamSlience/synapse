...
description: How Synapse connects references and links routes to handlers.
---
...
After parsing, Synapse resolves:
...
Synapse recognizes web-framework routing files and emits `route` nodes linked by `references` edges to their handler classes or functions — so querying the callers of a view or controller surfaces the URL pattern that binds it. See [Framework Routes](/synapse/guides/framework-routes/) for the full list of recognized frameworks.

## Dynamic-dispatch coverage

Static parsing misses computed and indirect calls, so flows can break at dynamic dispatch. Synapse bridges several of these boundaries with synthesizers so a flow connects end-to-end:
