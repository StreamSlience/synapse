...
// directly (open a DB, run prepared queries) rather than through the Synapse
...
/**
 * Options for initializing a new Synapse project
 */
...
/**
 * Options for opening an existing Synapse project
 */
...
export class Synapse {
   ...
   *
   * Creates the .synapse/ directory, database, and configuration.
   *
   * @param projectRoot - Path to the project root directory
   * @param options - Initialization options
   * @returns A new Synapse instance
   */
  ...
  /**
   * Open an existing Synapse project
   *
   * @param projectRoot - Path to the project root directory
  ...
  /**
   * Check if a directory has been initialized as a Synapse project
   */
  ...
  /**
   * Close the Synapse instance and release resources
   */
  close(): void {
    this.unwatch();
  ...
  async indexAll(options: IndexOptions = {}): Promise<IndexResult> {
    return this.indexMutex.withLock(async () => {
      ...
      try {
        ...
        // Stamp the index with the engine that built it, so `synapse status`
        // and `synapse upgrade` can recommend a re-index when the running
   ...
   * freshness without shelling out to `synapse status --json`. (#329)
   */
   ...
   * stamp is current. This is the signal behind `synapse status`'s re-index
   * hint and `synapse upgrade`'s reminder.
   */
   ...
   * built-in real-SQLite module). Surfaced via `synapse status` and the
   * `synapse_status` MCP tool alongside the effective journal mode.
   */
   ...
   * #238. Surfaced via `synapse status` and the `synapse_status` MCP tool.
   */
   ...
   * `synapse_explore` responses on small realworld template repos
   * (rails-realworld, laravel-realworld, drupal-admintoolbar, …) where
   * Glob+Read of `routes.rb`/`urls.py`/etc. otherwise beats synapse.
   */
  ...
  /**
   * Completely remove Synapse from the project.
   * This closes the database and deletes the .synapse/ directory.
   *
   * WARNING: This permanently deletes all Synapse data for the project.
   */
  uninitialize(): void {
    this.close();
