 ...
 *     as Claude.
 *   - Instructions to `./.cursor/rules/synapse.mdc` (project-local
 ...
 * `workspaceFolders` in the MCP initialize call. The synapse MCP
 * server's `process.cwd()` fallback therefore misses the workspace's
 * `.synapse/` and reports "not initialized" on every tool call.
 *
...
class CursorTarget implements AgentTarget {
  ...
  install(loc: Location, _opts: InstallOptions): WriteResult {
    ...
    // We no longer write `.cursor/rules/synapse.mdc` — the synapse
...
/**
 * Build the synapse MCP-server config for Cursor at the given
 ...
 *
 * Unlike the shared CLAUDE.md / AGENTS.md files (where synapse owns
 * only a marker-delimited section), `.cursor/rules/synapse.mdc` is a
 ...
 * block but leave the orphaned `description: Synapse ...` frontmatter
 * behind, so the file lingers and still "mentions" synapse.
 *
