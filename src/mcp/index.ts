 ...
 * Synapse MCP Server
 *
 * Model Context Protocol server that exposes Synapse functionality
 ...
 *   behavior; used when the user opts out (`SYNAPSE_NO_DAEMON=1`), no
 *   `.synapse/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one Synapse + watcher + SQLite handle. Spawned on demand; never a
...
// are cheap. Paired with deferring the Synapse load (engine.ts) off the bind
...
/** Whether `SYNAPSE_NO_DAEMON` was set to a truthy value. */
 ...
 * `null` when no `.synapse/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.synapse/`.
 *
 ...
 * `.synapse/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
 ...
 *
 * Implements the Model Context Protocol to expose Synapse
...
export class MCPServer {
   ...
   * `SYNAPSE_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
