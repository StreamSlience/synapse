 ...
 * see watch-policy.ts), the Synapse index would otherwise go stale until
 * the user runs `synapse sync` by hand. As an opt-in alternative, we can
 ...
 *
 * The hooks run `synapse sync` in the background so they never block git,
 * and are guarded by `command -v synapse` so they no-op cleanly when the
...
function markerBlock(): string {
  return [
    MARKER_BEGIN,
    '# Keeps the Synapse index fresh while the live file watcher is off',
    '# (e.g. WSL2 /mnt drives). Runs in the background so it never blocks git.',
    '# Managed by synapse; remove with `synapse uninit` or delete this block.',
...
/**
 * Install (or update) the Synapse sync hooks in a git repository.
...
/**
 * Remove the Synapse sync hooks. Strips only our marker block; deletes the
...
/** Whether any Synapse sync hook is currently installed. */
